...
class Wav2Vec2LayerNormConvLayer(nn.Module):
	def __init__(self, config, layer_id=0):
		...
		self.conv = nn.Conv1d(self.in_conv_dim, self.out_conv_dim,
			kernel_size=config.conv_kernel[layer_id],
			stride=config.conv_stride[layer_id],
			bias=config.conv_bias,
		)
		...
		if self.config.feat_enc_adapter:
			# self.convadapter = ConvAdapter(config=config, in_channels=self.out_conv_dim, out_channels=8)  #type-A
			# self.convadapter = ConvAdapter(config=config,						#line 32 setting, A kind of type-B
			# 					in_channels=self.in_conv_dim, 
			# 					out_channels=self.out_conv_dim,
			# 					kernel_size = config.conv_kernel[layer_id],
			# 					stride=config.conv_stride[layer_id],
			# 					bias=config.conv_bias,)
			if layer_id == 0:												  # A kind of type-B
				self.convadapter = ConvAdapter(config=config,
								in_channels=self.in_conv_dim, 
								out_channels=self.out_conv_dim,
								kernel_size = config.conv_kernel[layer_id],
								stride=config.conv_stride[layer_id],
	...
	def forward(self, hidden_states_input):
		...
		if self.config.feat_enc_adapter:
			# print("hidden_states	:", hidden_states.size())	 #[64,1,40000]
			# print("hidden_states_std:", hidden_states_std.size()) #[64,512,7999]
			# print("in_conv_dim	  :", self.in_conv_dim)	 #1
			# print("out_conv_dim	 :", self.out_conv_dim)   #512
...
class Wav2Vec2GroupNormConvLayer(nn.Module):
	def __init__(self, config, layer_id=0):
		...
		self.config = config
		if self.config.feat_enc_adapter:
			self.convadapter = ConvAdapter(config=config, in_channels=self.out_conv_dim, out_channels=8)
...
class Wav2Vec2Attention(nn.Module):
	...
	def __init__(
		...
		if config.lora_adapter:
			self.k_proj = lora.Linear(embed_dim, embed_dim, r=8)
			self.v_proj = lora.Linear(embed_dim, embed_dim, r=8)
			self.q_proj = lora.Linear(embed_dim, embed_dim, r=8)
			self.out_proj = lora.Linear(embed_dim, embed_dim, r=8)
		else:
			self.k_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
...
class Wav2Vec2FeedForward(nn.Module):
	def __init__(self, config):
		...
		if config.lora_adapter:
			self.intermediate_dense = lora.Linear(config.hidden_size, config.intermediate_size, r=8)
		else:
		...
		if config.lora_adapter:
			self.output_dense = lora.Linear(config.intermediate_size, config.hidden_size, r=8)
		else:
...
if __name__ == "__main__":
	model = Wav2Vec2ForSequenceClassification.from_pretrained("jonatasgrosman/wav2vec2-large-xlsr-53-english", num_labels=5)
	processor = Wav2Vec2Processor.from_pretrained("jonatasgrosman/wav2vec2-large-xlsr-53-english")

	audio_file = "./dataset/ESD/en/0013/Angry/train/0013_000493.wav"
	...
	label = torch.LongTensor([emo_label.index(str(audio_file.split(',')[0].split('/')[-3]))])

	output = model(input_values = inputs.input_values, attention_mask = inputs.attention_mask, labels=label)
	print(output.loss)
