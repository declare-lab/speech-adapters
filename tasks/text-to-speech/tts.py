...
def freeze_exclude_prompt(model):
	"""
...
def main(cfg: FairseqConfig) -> None:
	...
	# args_overrides = {'config_yaml': 'config.yaml', 'vocoder': 'griffin_lim', 'fp16': False, 'data': '/home/yingting/PromptSpeech/tasks/text-to-speech/vctk_transformer_phn'}
	# state = load_checkpoint_to_cpu("vctk_transformer_phn/checkpoint_best.pt", args_overrides)
	...
	# model = model.from_pretrained("vctk_transformer_phn/", checkpoint_file="checkpoint_best.pt")

	print(model)
	#######
	pretrained_dict=torch.load("ljspeech_transformer_phn/checkpoint_avg_last_5.pt")['model']
	...
	####TODO:
	## 1. load checkpoint
	## 2. modify archiecture of tts_transformer
	## 3. finetune
	## 4. metric
...
def cli_main(
	...
	# parser.add_argument_group('out_adapter', type=bool, default=False, help='add adapter before FFN')
